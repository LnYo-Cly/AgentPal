...
| --- | --- | --- | --- |
| Should a reviewer subagent be used? | no | 单文件 CLI 行为，可用 mock registry 和 targeted checks 覆盖。 | coordinator 自检。 |
| Would a worker subagent materially help? | no | 实现和验证集中，拆分会增加协调成本。 | 不申请 worker。 |
...
| --- | --- | --- | --- | --- | --- | --- |
| worker subagent | not-needed | coordinator | 2026-06-10 | no worker split | same checkout / master | 单文件 CLI 改动。 |
...
| Subagent 模式 | none | 任务范围很小。 |
| 审查模型 | self-check + mock verifier | 使用 mock registry 验证新版本提示和失败静默。 |
| Worktree 策略 | same checkout | 无 worker，当前 checkout 干净。 |
| 冲突控制 | coordinator owns shared files | subagent 不得直接编辑 coordinator 管理的全局表或共享文件，除非获得明确锁。 |
| 证据深度 | L1 | 覆盖 CLI help、mock latest、404 静默、关闭开关和 Harness check。 |
...
| --- | --- | --- | --- |
| L0 | `npm run agentpal -- --help` | `progress.md` | help 不出现更新提示。 |
| L1 | mock registry + `agentpal host --help`; npm 404 静默；`AGENTPAL_NO_UPDATE_CHECK=1`; `npm --prefix apps/mobile run typecheck`; `git diff --check`; `harness check --profile target-project .` | `progress.md` | 全部通过。 |
| L2 | 不适用 | n/a | 本任务不改真实 Relay/Host 协议。 |
| L3 | 不适用 | n/a | 不执行真实发布。 |
