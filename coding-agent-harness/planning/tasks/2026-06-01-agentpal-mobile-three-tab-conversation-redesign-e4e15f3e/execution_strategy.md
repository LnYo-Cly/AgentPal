...
| --- | --- | --- | --- |
| Should a reviewer subagent be used? | no | 本轮是单入口移动端 UI 重构，验证以 typecheck、静态检查和用户手机确认优先；不需要并行 reviewer。 | self-check |
| Would a worker subagent materially help? | no | 写入范围集中在 `apps/mobile/app/index.tsx` 和主题文件，拆分会增加冲突成本。 | coordinator 串行执行 |
...
| --- | --- | --- | --- | --- | --- | --- |
| worker subagent | not-needed | coordinator | 2026-06-01 | n/a | n/a | 单入口 UI 重构，不使用 worker subagent。 |
...
| Subagent 模式 | none | 单入口 UI 重构由 coordinator 直接完成。 |
| 审查模型 | self-check | 当前代码风险通过 typecheck、diff check 和 harness status 覆盖；视觉由用户手机确认。 |
| Worktree 策略 | same checkout | 无并行 worker，不需要额外 worktree。 |
| 冲突控制 | coordinator owns shared files | subagent 不得直接编辑 coordinator 管理的全局表或共享文件，除非获得明确锁。 |
| 证据深度 | L1 | 覆盖 typecheck、diff check、harness status；真机美感进入人工 review。 |
...
| L1 | `harness status --json .` | `progress.md` | 无 failure；提交前 dirty warning 可接受 |
| L2 | 用户手机视觉确认 | `review.md` | review 阶段人工确认或退回 |
