...
| --- | --- | --- | --- |
| Should a reviewer subagent be used? | no | 本轮是窄范围 SSoT 文档更新，self review + harness status 足够。 | 不调用 reviewer。 |
| Would a worker subagent materially help? | no | 没有可并行代码切片；拆分会增加共享文档冲突。 | coordinator 串行完成。 |
...
| --- | --- | --- | --- | --- | --- | --- |
| worker subagent | not-needed | coordinator | 2026-06-01 | SSoT docs only | same checkout | 文档小切片，不需要 worker。 |
...
| 审查模型 | self-check | 窄范围文档决策，验证重点是 SSoT 一致性和 harness 状态。 |
| Worktree 策略 | same checkout | 没有并行 worker 和代码实现。 |
| 冲突控制 | coordinator owns shared files | subagent 不得直接编辑 coordinator 管理的全局表或共享文件，除非获得明确锁。 |
| 证据深度 | L0 | 文档 SSoT 更新，用静态检查和 harness status 证明材料完整。 |
...
| --- | --- | --- | --- |
| L0 | `rg` 关键字检查、`git diff --check`、`harness status --json .` | `progress.md` | 文档包含 Live Surface 规则，Git diff 无空白问题，harness 无失败。 |
| L1 | 不适用 | n/a | 非代码实现。 |
| L2 | 不适用 | n/a | 非运行时实现。 |
| L3 | 不适用 | n/a | 非发布前平台能力。 |
