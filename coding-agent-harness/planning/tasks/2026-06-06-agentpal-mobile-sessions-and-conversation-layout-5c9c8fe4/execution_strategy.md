...
| --- | --- | --- | --- |
| Should a reviewer subagent be used? | no | 本轮是单文件 UI 布局修正，主要风险可通过自检、截图反馈和静态验证覆盖。 | 采用 coordinator self-check，并在 `review.md` 落盘。 |
| Would a worker subagent materially help? | no | 会话索引和详情页共享同一个入口文件，拆分会增加冲突和上下文同步成本。 | coordinator 单线程实现。 |
...
| --- | --- | --- | --- | --- | --- | --- |
| worker subagent | not-needed | coordinator | 2026-06-06 | single-file mobile UI layout correction | current checkout | 不使用 worker subagent。 |
...
| Subagent 模式 | none | 单文件 UI 修正，coordinator 负责实现和收口。 |
| 审查模型 | self-check | 风险集中在布局和类型安全，静态验证加用户真机复测足够。 |
| Worktree 策略 | same checkout | 当前工作区干净，不使用 worker。 |
| 冲突控制 | coordinator owns shared files | subagent 不得直接编辑 coordinator 管理的全局表或共享文件，除非获得明确锁。 |
| 证据深度 | L1 | TypeScript、Expo export、Harness check 覆盖移动端静态风险。 |
...
| L1 | `npm --prefix apps/mobile run typecheck`; `npx expo export --platform ios --output-dir ../../tmp/expo-export-layout-correction --clear` | `progress.md` | 命令通过。 |
| L1 | `npx --yes coding-agent-harness check --profile target-project .` | `progress.md` | 无 failure。 |
| L2 | 用户 iOS/Android Expo Go 真机复测 | `review.md` residual | 本地 Windows 环境无法代替真机视觉确认。 |
