...
AgentPal 的当前公开命令、帮助文案和移动端设备名统一为 `agentpal` / `AgentPal`，不再把 `oap` 或 `OpenAgentPal` 作为用户入口。

## 完成后能得到什么

完成后，下一轮 npm 发布准备可以围绕一个一致的产品名推进：包名 `agentpal`、公开命令 `agentpal`、产品文案 `AgentPal`。当前代码不会再引导用户使用 `oap` 或 `openagentpal`。Railway 平台域名和历史 Harness 审计记录保持不动，避免破坏现网连接和任务追溯。

## 交付物

- 可见产物：`agentpal pair --workspace .` 成为 CLI help 和源码态脚本主路径。
- 修改位置：`package.json`、`bin/oap.mjs`、`apps/mobile/src/hooks/useAgentPalRelay.ts`、当前开发上下文文档、设计文档。
...
- 范围内：当前公开 CLI 名称、help 文案、移动端 deviceName、开发上下文里仍指向 `oap` 的活跃说明。
