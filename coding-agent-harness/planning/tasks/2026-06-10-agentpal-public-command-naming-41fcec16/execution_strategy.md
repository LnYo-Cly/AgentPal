...
| --- | --- | --- | --- |
| Should a reviewer subagent be used? | no | 改动集中在少量命令入口和文案，验证可由 targeted checks 覆盖。 | coordinator 自检并记录证据。 |
| Would a worker subagent materially help? | no | 写入范围很小，拆分会增加任务文档和共享文件协调成本。 | 不申请 worker 授权。 |
...
| --- | --- | --- | --- | --- | --- | --- |
| worker subagent | not-needed | coordinator | 2026-06-10 | no worker split | same checkout / master | 用户要求当前直接改名；任务不需要并行 worker。 |
...
| Subagent 模式 | none | 小范围品牌/命令改名，不需要 subagent。 |
| 审查模型 | self-check + targeted verifier | CLI help、mobile typecheck、diff check 和 Harness check 足够覆盖风险。 |
| Worktree 策略 | same checkout | 当前 checkout 干净，且没有 worker subagent。 |
| 冲突控制 | coordinator owns shared files | subagent 不得直接编辑 coordinator 管理的全局表或共享文件，除非获得明确锁。 |
| 证据深度 | L1 | 覆盖 CLI help、脚本入口、mobile typecheck 和 Harness check。 |
...
| --- | --- | --- | --- |
| L0 | `rg` 检查剩余 `oap` / `OpenAgentPal` 当前表面引用 | `progress.md` | 只剩允许的历史、Relay 或兼容项。 |
| L1 | `npm run agentpal -- --help`; `npm --prefix apps/mobile run typecheck`; `git diff --check`; `harness check --profile target-project .` | `progress.md` | 全部通过。 |
| L2 | 不适用 | n/a | 本任务不改网络协议或运行时连接。 |
| L3 | 不适用 | n/a | 不执行真实发布。 |
