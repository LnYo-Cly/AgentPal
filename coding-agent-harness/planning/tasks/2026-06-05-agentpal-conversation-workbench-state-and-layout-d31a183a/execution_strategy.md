...
| --- | --- | --- | --- |
| Should a reviewer subagent be used? | no | 当前切片集中在 `apps/mobile/app/index.tsx` 的状态刷新、面板布局和列表密度，验证可由类型检查、Expo 导出和人工手机截图反馈覆盖。 | coordinator 自检并记录证据。 |
| Would a worker subagent materially help? | no | 改动集中在同一移动端入口文件，并行写同一 UI 文件会增加冲突成本。 | coordinator 单线实施。 |
...
| --- | --- | --- | --- | --- | --- | --- |
| worker subagent | not-needed | coordinator | 2026-06-05 | mobile conversation workbench UI/state repair | master | 单文件集中修复，不拆 worker。 |
...
| 审查模型 | self-check | 类型检查、Expo 导出和用户真机反馈足以覆盖本次 UI/state 修复。 |
| Worktree 策略 | same checkout | 未使用 worker；只在当前 checkout 修改受控文件。 |
| 冲突控制 | coordinator owns shared files | subagent 不得直接编辑 coordinator 管理的全局表或共享文件，除非获得明确锁。 |
| 证据深度 | L1 + L2 | 移动端 UI 状态修复需要静态类型检查和 Expo bundle 验证。 |
...
| L2 | `npx expo export --platform ios --output-dir tmp/expo-export-check --clear` | `progress.md` 或 `artifacts/INDEX.md` | Expo bundle 可导出，Expo Go 可重新加载。 |
| L3 | 用户 iPhone / Android 真机截图复核 | `review.md` 与 walkthrough | 用户确认关键交互可用后进入人工确认。 |
