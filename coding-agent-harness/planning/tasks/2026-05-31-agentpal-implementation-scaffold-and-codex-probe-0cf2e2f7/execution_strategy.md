...
| --- | --- | --- | --- |
| Should a reviewer subagent be used? | no | 本轮是初始脚手架和真实探测，风险主要由本地命令验证覆盖；暂不需要独立 reviewer。 | self-check 后提交 `review.md`。 |
| Would a worker subagent materially help? | no | 文件边界虽多但耦合在同一初始架构内，worker worktree 会增加集成成本。 | coordinator 单线执行。 |
...
| --- | --- | --- | --- | --- | --- | --- |
| worker subagent | not-needed | coordinator | 2026-05-31 | n/a | same checkout | 本轮不拆 worker。 |
...
| Subagent 模式 | none | coordinator 单线执行足够。 |
| 审查模型 | self-check | 初始脚手架以编译、typecheck、真实 probe 和 harness status 为主证据。 |
| Worktree 策略 | same checkout | 无 worker subagent，直接在当前 checkout 实现。 |
| 冲突控制 | coordinator owns shared files | subagent 不得直接编辑 coordinator 管理的全局表或共享文件，除非获得明确锁。 |
| 证据深度 | L2 | 需要覆盖静态检查、编译、服务冒烟和真实 Codex probe。 |
...
| --- | --- | --- | --- |
| L0 | `cargo fmt --all`、移动端 TypeScript 检查 | `progress.md` | 命令退出码为 0。 |
| L1 | `cargo check --workspace` | `progress.md` | Rust workspace 编译通过。 |
| L2 | Relay `/healthz`、Host `codex probe` 真实探测、`harness status --json .` | `progress.md`、`review.md` | Relay 可用；Codex probe 输出真实结构化结果；harness 状态通过。 |
| L3 | 不适用 | `review.md` | 本轮不发布生产服务。 |
